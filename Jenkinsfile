pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                echo 'Build'
                sh 'docker build --no-cache -t europe-southwest1-docker.pkg.dev/banded-coder-401610/cloudmss/log4java:2 .'
            }
        }
         ...
         stage('push gcr') {
            steps {
                ...
                withCredentials([file(credentialsId: 'gcloud-creds', variable: 'GCLOUD_CREDS')]){
                    ...
                    '''
                    sh '''
                    sudo docker push europe-southwest1-docker.pkg.dev/banded-coder-401610/cloudmss/log4java:2
                    '''
                }                           
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker image rm europe-southwest1-docker.pkg.dev/banded-coder-401610/cloudmss/log4java:2'
            // docker logout
            sh 'docker logout'
